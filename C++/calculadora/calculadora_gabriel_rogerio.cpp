...
#include<string.h>
#include<ctype.h>
//calculadora de mals habitos, FTEC
//trabalho de Gabriel Hofman && Rogerio Lazzari
...
main (void)
{
	...
	do
	{
		...
		do
		{
			...
			scanf("%d", &codbebidas);
				...
				case 7:
					printf("\nQuantas taças de espumante bebe  por semana? (considerando 125 ml a taça): ");
	...
	switch(charcigarro)
	{
		...
		case 'S':
			system("cls");
			printf("\nQuantas carteiras de cigarros fuma por semana?(considere 20 cigarros a carteira)");
			scanf("%f", &qcigarros);
	...
	system("cls");
		...
		do
		{
			...
			if(qcigarros>0)
			{
				printf("\nNumero de Cigarros: %.0f Unidades",((qcigarros*52)*anos)*20);
			}
			...
			if(tespulmantes>0)
			{
				printf("\nEspumantes: %.2f R$",tespulmantes*anos);
			}
			...
			if(toxi>0)
			{
				...
				do
				{
					printf("\n\n\nDeseja modificar os anos para calculo?('S' para afirmativo, e 'N' para negativo)");
					fflush(stdin);
					v2=toupper(getche());
